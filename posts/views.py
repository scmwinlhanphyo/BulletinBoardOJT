...
def index(request):
    ...
    post_list = Posts.objects.filter(query)
...
def userList(request):
    ...
    user_list = Users.objects.filter(query)

    for user_data in user_list:
        user_data.type = 'Admin' if user_data.type == '0' else 'User'
        list_data = Users.objects.filter(id=user_data.created_user_id)
        user_data.created_user = list_data[0].name if len(list_data) > 0 else ''
...
def user_login(request):
    ...
    if request.method == 'POST':
        ...
        email_user = Users.objects.filter(email=email, deleted_user_id=None, deleted_at=None)
        if email_user:
...
def post_update(request, pk):
    user = get_object_or_404(Users, pk=request.user.id)
    detail_post = get_object_or_404(Posts, pk=pk)
        ...
        if "_save" in request.POST:
            ...
            if form.is_valid():
                ...
                if request.session["create_update_confirm_page_flag"] == True:
                    ...
                    edit_post = get_object_or_404(Posts, pk=detail_post.id)
                    edit_post.title = form.cleaned_data.get('title')
                    edit_post.description = form.cleaned_data.get('description')
...
def post_detail(request):
    ...
    obj = Posts.objects.get(pk=post_id)
    created_user_name = obj.user.name
    updated_user = Users.objects.get(pk=obj.updated_user_id)
...
def user_detail(request):
    ...
    obj = Users.objects.get(pk=user_id)
    profile_url = ''
    if obj.profile and hasattr(obj.profile, 'url'):
        profile_url = obj.profile.url
    else:
        profile_url = 'media/user-default.png'
    created_user = Users.objects.get(pk=obj.created_user_id) if obj.created_user_id != None else None
    updated_user = Users.objects.get(pk=obj.updated_user_id) if obj.updated_user_id != None else None
    data = serializers.serialize('json', [obj,])
    struct = json.loads(data)
...
def user_update(request, pk):
    req_user = get_object_or_404(Users, pk=pk)
    profile = req_user.profile if req_user.profile.url and hasattr(req_user.profile, 'url') else ''
    check_route('user', request.META.get('HTTP_REFERER'), request)
    formData = {
        'name': req_user.name,
                ...
                if request.session.get("create_update_confirm_page_flag") == True:
                    try:
                        ...
                        if oldFileDir != request.session.get('profile'):
                            ...
                            remove_temp(file_name, ROOT_DIR)
                    ...
                    if 'profile' in request.FILES:
                        profile = save_temp(request.FILES['profile'])
                        request.session["profile"] = '/static/temp/' + profile
                    else:
...
def user_profile(request):
    current_user = get_object_or_404(Users, pk=request.user.id)
    current_user.profile = current_user.profile.url if current_user.profile and hasattr(current_user.profile, 'url') else '/media/user-default.png'
    context = {
        "id": current_user.id,
...
def download_post_list_csv(request):
    ...
    writer.writerow(['id', 'title', 'description', 'status', 'created_user_id', 'updated_user_id', 'deleted_user_id', 'deleted_at', 'created_at', 'updated_at'])

    for post in post_list:
        writer.writerow([post.id, post.title, post.description, post.status, post.created_user_id, post.updated_user_id, post.deleted_user_id, post.deleted_at, post.created_at, post.updated_at])
    return response
...
def signup(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            new_user.save()
            authUser = authenticate(request, username=form.cleaned_data.get('email'), password=form.cleaned_data.get('password'))
            print('authUser')
            print(authUser)
            if authUser:
                print('authUser')
                print(authUser)
                login(request, authUser)
                request.session['login_username'] = form.cleaned_data.get('name')
...
def post_delete(request):
    post_id = request.GET['post_id']
    delete_post = get_object_or_404(Posts, pk=post_id)
...
def user_delete(request):
    user_id = request.GET['user_id']
    delete_user = get_object_or_404(Users, pk=user_id)
